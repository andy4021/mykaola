...
SlideShow.prototype.initUI = function(){
        ...
        for(let i=0;i<this.imgs.length;i++){
            ...
            //2)、设置样式
            imgDom.style.cssText = "position:absolute;left:"+this.left+"px;top:"+this.top+"px;width:"+this.width+"px;height:"+this.height+"px;";
            //3)、添加到容器里
            this.boxDom.appendChild(imgDom);
        }
        //2、创建ul
        let ulDom = document.createElement("ul");
        ulDom.style.cssText = "position:absolute;right:"+this.btnRight+"px;bottom:"+this.btnBottom+"px;height:40px;";
        //3、创建li
        for(let i=0;i<this.imgs.length;i++){
...
SlideShow.prototype.initEvent = function(){
        ...
        for(let i=0;i<lis.length;i++){
            //赋值语句。
            lis[i].onmouseover = function(){//此函数的执行必须点击li。
                obj.goImg(i);//
            }
        }
    }
